//*****************************************************************************
//
// enet_tcpecho_server_static_addrress.c - Sample Echo Server Application
// using lwIP with static IP address.
//
...
//! <h1>Ethernet TCP Echo Server Using Static Address
//! (enet_tcpecho_server_static_addrress)</h1>
//!
//! This example application demonstrates the operation of the TM4C129x
//! Ethernet controller using the lwIP TCP/IP Stack.  The static IP address
//! is manually provided during compile time. This example is very much the
//! same as the enet_tchpecho_server which relies on the DHCP server to acquire
//! the dynamic IP address. The address that is selected will be shown on the
//! UART.  The application echoes back the data received from the client with
//! the inverted case.
//!
//! To enable LwIP for static IP address operation, the lwipopts.h file must be
//! changed for the below two defines before compiling. See lwipopts.h for
//! details.
//!
//! #define LWIP_DHCP                       0
...
//*****************************************************************************
//
// Defines for the static IP address, Network Mask and Gateway Mask. User
// must change the values per their local settings.
//
// Port 23 is chosen for telenet connection.
//
// TODO: User must change the below setting per their application
// If the IP address is AA.BB.CC.DD then:
// #define IPADDR DD.CC.BB.AA
//
//*****************************************************************************
#define IPADDR "206.212.130.10"
#define NETMASK "0.252.255.255"
#define GWMASK "0.252.255.255"
#define PORT 23
...
main(void)
{
    ...
    UARTprintf("Ethernet lwIP TCP echo example with static IP address.\n\n");
