...
lwIPInterruptTask(void *pvArg)
{
    ...
    while(1)
    {
        ...
        //
        MAP_EMACIntEnable(EMAC0_BASE, EMAC_INT_MASK);
    }
}
#endif
...
lwIPLinkDetect(void)
{
    ...
    uint16_t PHY1_Link=0;
    uint16_t PHY2_Link=0;
#endif
    ...
    PHY1_Link = (EMACPHYRead(EMAC0_BASE, 2, EPHY_BMSR) & EPHY_BMSR_LINKSTAT) >> 2;
    PHY2_Link = (EMACPHYRead(EMAC0_BASE, 3, EPHY_BMSR) & EPHY_BMSR_LINKSTAT) >> 2;
	if((PHY1_Link > 0) || (PHY2_Link > 0))
	{
		bHaveLink = TRUE;
	}
	else
...
void lwIPEthernetIntHandler(void)
{
    ...
    //
    MAP_EMACIntDisable(EMAC0_BASE, EMAC_INT_MASK);
...
//*****************************************************************************
// Check Link Detect Or NOt
// return bool true or false
//*****************************************************************************

bool CheckLinkDetect (void)
{
	lwIPLinkDetect();
